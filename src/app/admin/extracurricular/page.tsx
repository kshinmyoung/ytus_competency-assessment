  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-lg font-bold text-ys-ink">비교과 관리</h2>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(item.major_competency_tags ?? []).map((id) => {
                        ...
                        return comp ? <span key={`m-${id}`} className="rounded-full bg-ys-blue/10 px-1.5 py-0.5 text-[10px] font-medium text-ys-blue">{comp.name}</span> : null;
                      })}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${item.is_active ? "bg-ys-gold/15 text-[#8A6212]" : "bg-slate-100 text-ys-ink-soft"}`}>
                        {item.is_active ? "활성" : "비활성"}
                      </span>
                      {item.registration_open && <span className="rounded-full bg-ys-blue/10 px-2 py-0.5 text-xs font-medium text-ys-blue">모집중</span>}
                    </div>
                  </td>
      ...
      {showParticipants && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowParticipants(null)}>
          <div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-2 text-[10px] text-ys-ink-soft/70">CSV 헤더: student_id, status (신청/참여중/완료)</p>
            {participants.length === 0 ? (
              <p className="mt-4 text-sm text-ys-ink-soft">참여 학생이 없습니다.</p>
            ) : (
              <table className="mt-4 min-w-full divide-y divide-slate-200">
                <thead className="bg-ys-paper">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-semibold text-ys-ink-soft">상태</th>
                    <th className="px-4 py-2 text-left text-xs font-semibold text-ys-ink-soft">소감</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {participants.map((p) => (
                    <tr key={p.student_id}>
                      <td className="px-4 py-2 text-sm text-ys-ink">{p.student_id}</td>
                      <td className="px-4 py-2 text-sm text-ys-ink-soft">{p.students?.name ?? "-"}</td>
                      <td className="px-4 py-2">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                          p.status === "완료" ? "bg-ys-gold/15 text-[#8A6212]" : p.status === "참여중" ? "bg-ys-blue/10 text-ys-blue" : "bg-ys-blue/10 text-ys-blue"
                        }`}>{p.status}</span>
                      </td>
                      <td className="px-4 py-2 text-sm text-ys-ink-soft">{p.reflection ? p.reflection.substring(0, 50) + (p.reflection.length > 50 ? "..." : "") : "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            )}
            <button type="button" onClick={() => setShowParticipants(null)} className="mt-4 w-full rounded-lg bg-slate-200 py-2 text-sm font-medium text-ys-ink hover:bg-slate-300">닫기</button>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowForm(false)}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{editingId ? "프로그램 수정" : "프로그램 추가"}</h3>
            <form onSubmit={handleSave} className="mt-4 space-y-4">
              ...
              <div><label className="block text-sm font-medium text-ys-ink">설명</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
              <div className="flex gap-6">
                ...
                <label className="flex items-center gap-2 text-sm text-ys-ink"><input type="checkbox" checked={form.registration_open} onChange={(e) => setForm({ ...form, registration_open: e.target.checked })} className="rounded" /> 모집 중</label>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {majorComps.map((comp) => (
                    <button key={comp.id} type="button" onClick={() => toggleTag("major_competency_tags", comp.id)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${form.major_competency_tags.includes(comp.id) ? "bg-ys-blue/15 text-ys-blue ring-2 ring-indigo-400 ring-offset-1" : "bg-ys-blue/10 text-ys-sky opacity-50"}`}>
                      {comp.name}
                    </button>
                  ))}
                </div>
      ...
      {showStudentCsv && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowStudentCsv(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-semibold text-ys-ink">학생-비교과 일괄 연결</h3>
            <p className="mt-1 text-sm text-ys-ink-soft">헤더: student_id, extracurricular_id, status</p>
            <p className="mt-1 text-xs text-ys-ink-soft/70">extracurricular_id 대신 extracurricular_name(프로그램명)도 사용 가능. status: 신청/참여중/완료</p>
            {csvProcessing && <p className="mt-3 text-sm text-ys-blue">처리 중...</p>}
            {csvResult && (
              <div className="mt-3 rounded-lg border border-slate-200 bg-ys-paper p-3">
                <p className="text-sm">성공: <strong className="text-[#8A6212]">{csvResult.success}건</strong>, 실패: <strong className="text-red-600">{csvResult.failed}건</strong></p>
