  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-lg font-bold text-ys-ink">설문조사 관리</h2>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={() => setFilterOrg("all")} className={`rounded-full px-3 py-1 text-xs ${filterOrg === "all" ? "bg-slate-800 text-white" : "border border-slate-300 bg-white text-ys-ink-soft"}`}>전체</button>
        {ORG_TYPES.flatMap((g) => g.orgs).map((name) => (
          <button key={name} type="button" onClick={() => setFilterOrg(name)} className={`rounded-full px-3 py-1 text-xs ${filterOrg === name ? "bg-slate-800 text-white" : "border border-slate-300 bg-white text-ys-ink-soft"}`}>{name}</button>
        ))}
      </div>

      {/* 설문 목록 */}
      <div className="space-y-3">
        {filtered.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center"><p className="text-sm text-ys-ink-soft">설문이 없습니다.</p></div> :
          filtered.map((s) => (
            <div key={s.id} className={`rounded-xl border border-slate-200 bg-white p-5 shadow-sm ${!s.is_active ? "opacity-50" : ""}`}>
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${s.org_type === "department" ? "bg-ys-blue/10 text-ys-blue" : "bg-ys-gold/15 text-[#8A6212]"}`}>{s.org_name}</span>
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${s.is_active ? "bg-ys-gold/15 text-[#8A6212]" : "bg-slate-100 text-ys-ink-soft/70"}`}>{s.is_active ? "진행중" : "마감"}</span>
                  </div>
                  <h3 className="mt-1.5 text-sm font-semibold text-ys-ink">{s.title}</h3>
                  {s.description && <p className="mt-0.5 text-xs text-ys-ink-soft">{s.description}</p>}
                  {s.end_date && <p className="mt-0.5 text-[10px] text-ys-ink-soft/70">마감: {s.end_date}</p>}
                </div>
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => openQuestions(s)} className="rounded p-1.5 text-ys-ink-soft/70 hover:bg-slate-100 hover:text-ys-ink-soft" title="문항 관리"><Edit3 className="h-4 w-4" /></button>
                  <button type="button" onClick={() => openResponses(s)} className="rounded p-1.5 text-ys-blue hover:bg-ys-blue/10 hover:text-ys-blue" title="응답 조회"><Eye className="h-4 w-4" /></button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{editingId ? "설문 수정" : "설문 추가"}</h3>
            <form onSubmit={handleSave} className="mt-4 space-y-4">
              <div><label className="block text-sm font-medium text-ys-ink">제목 *</label><input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
              <div><label className="block text-sm font-medium text-ys-ink">설명</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="block text-sm font-medium text-ys-ink">소속 구분</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-ys-ink">소속</label>
                ...
                <div><label className="block text-sm font-medium text-ys-ink">마감일</label><input type="date" value={form.end_date} onChange={(e) => setForm({ ...form, end_date: e.target.value })} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
              </div>
              <label className="flex items-center gap-2 text-sm text-ys-ink"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="rounded" /> 활성</label>
      ...
      {showQuestions && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowQuestions(null)}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{showQuestions.title} - 문항 관리</h3>
            <p className="mt-1 text-xs text-ys-ink-soft">{questions.length}개 문항</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowResponses(null)}>
          <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-ys-ink">{showResponses.title} - 응답 ({responses.length}건)</h3>
              </div>
              <button type="button" onClick={downloadResponses} className="flex items-center gap-1.5 rounded-lg bg-ys-blue px-4 py-2 text-sm font-medium text-white hover:bg-ys-blue/90">
                <Download className="h-4 w-4" /> CSV 다운로드
              </button>
            </div>

            {responses.length === 0 ? (
              <p className="mt-6 text-center text-sm text-ys-ink-soft">응답이 없습니다.</p>
            ) : (
              <div className="mt-4 overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-ys-paper">
                    <tr>
                      <th className="px-3 py-2 text-left text-[10px] font-semibold text-ys-ink-soft">학번</th>
                      <th className="px-3 py-2 text-left text-[10px] font-semibold text-ys-ink-soft">이름</th>
                      {respQuestions.map((q) => <th key={q.id} className="px-3 py-2 text-left text-[10px] font-semibold text-ys-ink-soft max-w-[150px] truncate">{q.question_order}. {q.question_text}</th>)}
                      <th className="px-3 py-2 text-left text-[10px] font-semibold text-ys-ink-soft">제출일</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {responses.map((r) => (
                      <tr key={r.id} className="hover:bg-ys-paper">
                        <td className="px-3 py-2 text-xs text-ys-ink">{r.student_id}</td>
                        <td className="px-3 py-2 text-xs text-ys-ink-soft">{nameMap[r.student_id] ?? "-"}</td>
                        {respQuestions.map((q) => {
                          const val = r.answers[q.id];
                          return <td key={q.id} className="px-3 py-2 text-xs text-ys-ink max-w-[150px] truncate">
                            {q.question_type === "likert" ? (val ? `${val}점` : "-") : (val ?? "-")}
                          </td>;
                        })}
                        <td className="px-3 py-2 text-[10px] text-ys-ink-soft/70">{new Date(r.submitted_at).toLocaleDateString("ko-KR")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <button type="button" onClick={() => setShowResponses(null)} className="mt-4 w-full rounded-lg border border-slate-300 py-2 text-sm font-medium text-ys-ink hover:bg-ys-paper">닫기</button>
          </div>
        </div>
      )}
    </AdminLayout>
