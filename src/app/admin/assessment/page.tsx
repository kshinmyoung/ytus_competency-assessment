...
export default function AdminAssessmentPage() {
  ...
  const renderSubcategory = (dt: DiagnosisType, sub: Subcategory) => {
    ...
    return (
      <div key={sub.id} className="rounded-xl border border-slate-200 bg-ys-paper/50">
        <div className="flex items-center justify-between px-4 py-2.5">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-ys-ink">{sub.name}</span>
            {sub.department_id && <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[9px] text-ys-ink-soft">{deptNameMap[sub.department_id]}</span>}
            <span className="text-xs text-ys-ink-soft/70">{catQuestions.length}문항</span>
          </div>
          <div className="flex items-center gap-1">
            <button type="button" onClick={() => openAddQuestion(dt.key, sub.sort_order, sub.department_id)} className="flex items-center gap-1 rounded px-2 py-1 text-xs text-ys-blue hover:bg-ys-blue/10">
            ...
            <button type="button" onClick={() => {
              ...
              setShowSubForm(true);
            }} className="rounded p-1 text-ys-ink-soft/70 hover:text-ys-ink-soft">
              <Edit3 className="h-3.5 w-3.5" />
            </button>
            <button type="button" onClick={() => handleDeleteSub(sub)} className="rounded p-1 text-ys-ink-soft/70 hover:text-red-500">
              ...
              <tbody className="divide-y divide-slate-100">
                {catQuestions.map((q) => (
                  <tr key={q.id} className="hover:bg-white">
                    <td className="w-8 px-3 py-2 text-center text-xs text-ys-ink-soft/70">{q.question_order}</td>
                    <td className="px-3 py-2">
                      <p className="text-sm text-ys-ink">{q.question_text}</p>
                      <div className="mt-0.5 flex gap-1">
                        {(options.get(q.id) ?? []).map((opt) => (
                          <span key={opt.id} className="rounded bg-white px-1 py-0.5 text-[9px] text-ys-ink-soft/70 ring-1 ring-slate-200">{opt.option_value}점</span>
                        ))}
                      </div>
                    </td>
                    <td className="w-12 px-2 py-2 text-center">
                      <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-medium ${q.is_active ? "bg-ys-gold/15 text-[#8A6212]" : "bg-slate-100 text-ys-ink-soft/70"}`}>{q.is_active ? "ON" : "OFF"}</span>
                    </td>
                    <td className="w-20 whitespace-nowrap px-2 py-2 text-right">
                      <button type="button" onClick={() => openEditQuestion(q)} className="mr-1 text-ys-blue hover:text-ys-blue"><Edit3 className="inline h-3.5 w-3.5" /></button>
                      <button type="button" onClick={() => handleToggleQ(q)} className="mr-1 text-xs text-ys-ink-soft/70 hover:text-ys-ink-soft">{q.is_active ? "끄기" : "켜기"}</button>
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-lg font-bold text-ys-ink">역량진단 관리</h2>
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-ys-blue px-4 py-2 text-sm font-medium text-white hover:bg-ys-navy-soft"
          >
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <span className="text-sm text-ys-ink-soft">총 {questions.length}개 문항</span>
      </div>

      {/* 진단 유형별 아코디언 */}
      {filteredTypes.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          <p className="text-sm text-ys-ink-soft">등록된 진단 유형이 없습니다.</p>
        </div>
      ) : (
          ...
          {filteredTypes.map((dt) => {
            ...
            return (
              <div key={dt.key} className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                {/* 유형 헤더 */}
                <div
                  className="flex cursor-pointer items-center justify-between px-5 py-4 hover:bg-ys-paper"
                  onClick={() => toggleExpand(dt.key)}
                >
                  <div className="flex items-center gap-3">
                    {expanded ? <ChevronDown className="h-4 w-4 text-ys-ink-soft/70" /> : <ChevronRight className="h-4 w-4 text-ys-ink-soft/70" />}
                    <div className="h-3 w-3 rounded-full" style={{ backgroundColor: dt.color }} />
                    <div>
                      <h3 className="text-sm font-semibold text-ys-ink">{dt.label}</h3>
                      {dt.description && <p className="text-xs text-ys-ink-soft">{dt.description}</p>}
                    </div>
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-ys-ink-soft">{subs.length}개 카테고리 · {totalQ}문항</span>
                    {["core_diagnosis", "learning_diagnosis", "calling_diagnosis"].includes(dt.key) && (
                      <span className="rounded bg-ys-gold/10 px-2 py-0.5 text-[10px] text-[#8A6212]">기존 결과 보존</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                    <button type="button" onClick={() => {
                      ...
                      setShowTypeForm(true);
                    }} className="rounded p-1 text-ys-ink-soft/70 hover:bg-slate-100 hover:text-ys-ink-soft">
                      <Settings className="h-4 w-4" />
                    </button>
                    <button type="button" onClick={() => handleDeleteType(dt)} className="rounded p-1 text-ys-ink-soft/70 hover:bg-red-50 hover:text-red-500">
                  ...
                  <div className="border-t border-slate-100 px-5 pb-5 pt-3">
                    ...
                    <div className="mb-3 flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-1 rounded-lg border border-dashed border-slate-300 px-3 py-1.5 text-xs text-ys-ink-soft hover:border-ys-blue/50 hover:text-ys-blue"
                      >
                    ...
                    {subs.length === 0 ? (
                      <p className="py-4 text-center text-sm text-ys-ink-soft/70">하위 카테고리가 없습니다.</p>
                    ) : (
                      <div className="space-y-4">
                        {/* 학과별 그룹핑이 필요한 유형 */}
                        {hasDeptSubs(dt.key) && (() => {
                          ...
                          return deptIds.map((dId) => (
                            <div key={`dept-${dId}`} className="mb-2">
                              <p className="mb-2 rounded-md bg-ys-blue/10 px-3 py-1.5 text-xs font-semibold text-ys-blue">{deptNameMap[dId] ?? `학과 ${dId}`}</p>
      ...
      {showTypeForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowTypeForm(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{editingType ? "진단 유형 수정" : "진단 유형 추가"}</h3>
            <form onSubmit={handleSaveType} className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-ys-ink">유형 키 (영문) {editingType && <span className="text-xs text-ys-ink-soft/70">- 수정 불가</span>}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-ys-ink">표시 이름</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-ys-ink">색상</label>
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-xs text-ys-ink-soft">{typeForm.color}</span>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-ys-ink">정렬 순서</label>
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowTypeForm(false)} className="flex-1 rounded-lg border border-slate-300 py-2 text-sm font-medium text-ys-ink hover:bg-ys-paper">취소</button>
                <button type="submit" disabled={saving} className="flex-1 rounded-lg bg-ys-blue py-2 text-sm font-medium text-white hover:bg-ys-navy-soft disabled:opacity-50">{saving ? "저장 중..." : "저장"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showSubForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowSubForm(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{editingSub ? "카테고리 수정" : "카테고리 추가"}</h3>
            <p className="mt-1 text-xs text-ys-ink-soft">{typeMap[subForm.diagnosis_type_key]?.label}</p>
            <form onSubmit={handleSaveSub} className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-ys-ink">카테고리명</label>
              ...
              <div>
                <label className="block text-sm font-medium text-ys-ink">소속 학과 (선택)</label>
              ...
              <div>
                <label className="block text-sm font-medium text-ys-ink">순서</label>
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowSubForm(false)} className="flex-1 rounded-lg border border-slate-300 py-2 text-sm font-medium text-ys-ink hover:bg-ys-paper">취소</button>
                <button type="submit" disabled={saving} className="flex-1 rounded-lg bg-ys-blue py-2 text-sm font-medium text-white hover:bg-ys-blue/90 disabled:opacity-50">{saving ? "저장 중..." : "저장"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showQForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowQForm(false)}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-ys-ink">{editingQ ? "문항 수정" : "문항 추가"}</h3>
            ...
            <form onSubmit={handleSaveQ} className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-ys-ink">문항</label>
              ...
              {/* 보기(점수) 설정 */}
              <div>
                <label className="block text-sm font-medium text-ys-ink">보기 및 점수 설정</label>
                <div className="mt-2 space-y-2">
                  {qForm.options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="w-12 text-right text-xs text-ys-ink-soft">{opt.option_value}점</span>
                  ...
                  <button type="button" onClick={() => setQForm((p) => ({ ...p, options: [...p.options, { option_text: "", option_value: p.options.length + 1, option_order: p.options.length + 1 }] }))} className="text-sm text-ys-blue hover:underline">+ 보기 추가</button>
                </div>
              </div>

              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowQForm(false)} className="flex-1 rounded-lg border border-slate-300 py-2 text-sm font-medium text-ys-ink hover:bg-ys-paper">취소</button>
                <button type="submit" disabled={saving} className="flex-1 rounded-lg bg-ys-blue py-2 text-sm font-medium text-white hover:bg-ys-blue/90 disabled:opacity-50">{saving ? "저장 중..." : "저장"}</button>
              </div>
            </form>
          </div>
